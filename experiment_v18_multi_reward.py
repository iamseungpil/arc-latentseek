#!/usr/bin/env python3
"""
Experiment with V18 - 5D Multi-reward system based on CompressARC
"""
...
from src.optimizers.latent_optimizer_v18_multi_reward import LatentOptimizerV18MultiReward
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('experiment_v18.log')
    ]
)
logger = logging.getLogger(__name__)

def run_v18_experiment(
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    num_problems: int = 5,
    device: str = "cuda:5"
):
    """Run V18 experiment with 5D multi-reward system."""
    ...
    # Initialize V18 optimizer with multi-reward
    optimizer = LatentOptimizerV18MultiReward(
        model=model,
        tokenizer=tokenizer,
        evaluator=evaluator,
        learning_rate=0.03,
        num_steps=20,  # More steps for multi-reward learning
        temperature=1.0,
        max_new_tokens=1024,
    ...
    # Results storage
    results = {
        "experiment": "v18_multi_reward_5d",
        ...
        "config": {
            ...
            "learning_rate": 0.03,
            "temperature": 1.0,
            "reward_system": "5D CompressARC-style"
        },
        "problems": []
    }
    ...
    for i, problem_id in enumerate(problem_ids):
        ...
        # Run V18 optimization
        logger.info("Running V18 optimization with 5D multi-reward...")
        ...
        if opt_result["success"]:
            ...
            if opt_result["final_code"]:
                code_path = Path(f"results/v18/{problem_id}_final.py")
        ...
        with open("results/v18/results.json", 'w') as f:
    ...
    if completed:
        ...
        logger.info(f"\n{'='*80}")
        logger.info("V18 MULTI-REWARD EXPERIMENT COMPLETE")
        ...
        logger.info(f"Results saved to: results/v18")
        logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    run_v18_experiment(device="cuda:5")