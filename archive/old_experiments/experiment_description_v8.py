#!/usr/bin/env python3
"""
Description-Targeted V8 Experiment
Optimizes only the description part of generated code
"""
...
from src.optimizers.latent_optimizer_description_v8 import DescriptionTargetedOptimizerV8

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def run_description_v8_experiment(
    num_problems: int = 10,
    device: str = "cuda",
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    output_dir: str = "results/description_v8"
):
    """Run Description-Targeted V8 experiment"""
    ...
    optimizer = DescriptionTargetedOptimizerV8(
        barc_generator=generator,
        code_executor=executor,
        lr=0.01,
        max_steps=30,  # Fewer steps since we're targeting specific area
        # 5D weights
        accuracy_weight=0.3,
        color_weight=0.2,
        spatial_weight=0.2,
    ...
    # Results tracking
    results = {
        "experiment": "description_targeted_v8",
        "model": model_name,
    ...
    for i, problem in enumerate(problems):
        ...
        try:
            ...
            # Run V8 optimization
            logger.info("\nRunning Description-Targeted V8 optimization...")
            ...
            desc_path = output_path / f"{problem.uid}_descriptions.txt"
            with open(desc_path, 'w') as f:
                f.write("Description Evolution:\n")
                f.write("="*50 + "\n\n")
                for idx, desc in enumerate(opt_result.description_history):
                    f.write(f"Step {idx}: {desc}\n\n")
    ...
    logger.info(f"\n{'='*80}")
    logger.info("V8 EXPERIMENT COMPLETE")
