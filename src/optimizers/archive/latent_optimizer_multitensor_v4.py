"""
MultiTensor V4 - CompressARC-style VAE approach
Uses KL divergence + reconstruction error for optimization
"""
...
class MultiTensorOptimizerV4:
    """MultiTensor optimizer using CompressARC-style VAE approach"""
    ...
    def optimize(self, 
        ...
        """
        Optimize using CompressARC-style VAE approach
        """
        logger.info(f"Starting MultiTensor V4 optimization for problem {problem.uid}")
        ...
        # Fix: Handle batch dimension correctly
        ...
        for i in range(start_index, min(start_index + update_length, len(hidden_states_list))):
            ...
            if h.dim() == 2:  # [batch_size, hidden_dim]
                h = h.squeeze(0)  # Remove batch dimension -> [hidden_dim]
        ...
        for step in range(self.max_steps):
            ...
            if new_output and new_output.code:
                ...
                # KL divergence (L2 regularization as proxy)
                kl_loss = F.mse_loss(optimized_hidden_states, original_hidden_states)
                
                # Total loss (following CompressARC formula)
                loss = self.kl_weight * kl_loss + self.reconstruction_weight * reconstruction_error
                
                # Log details
                logger.info(f"Loss: {loss.item():.4f} (KL: {kl_loss.item():.4f}, Recon: {reconstruction_error.item():.4f})")
    ...
    def _calculate_reconstruction_error(self,
        ...
        """
        Calculate reconstruction error based on execution results
        Similar to CompressARC's cross-entropy approach
        """
        device = next(self.model.parameters()).device
        total_error = 0.0
        ...
        # Compare output grids with expected
        for i, (pair, output_grid) in enumerate(zip(problem.train_pairs, execution_result.output_grids)):
            ...
            if isinstance(output_grid, np.ndarray) and output_grid.shape == expected.shape:
                # Convert to tensors
                output_tensor = torch.tensor(output_grid, device=device, dtype=torch.long)
                expected_tensor = torch.tensor(expected, device=device, dtype=torch.long)
                
                # Cross-entropy style error
                # Create one-hot encoding for colors (0-9)
                num_colors = 10
                output_flat = output_tensor.flatten()
                expected_flat = expected_tensor.flatten()
                
                # Create logits (use one-hot with some noise for differentiability)
                logits = F.one_hot(output_flat, num_colors).float()
                logits = logits + 0.1  # Add small value to avoid log(0)
                logits = torch.log(logits / logits.sum(dim=-1, keepdim=True))
                
                # Cross-entropy
                error = F.cross_entropy(logits, expected_flat, reduction='mean')
                total_error += error
            else:
                # Shape mismatch penalty
                total_error += 5.0
        
        # Multi-dimensional evaluation bonus/penalty
        eval_result = self.evaluator.evaluate(problem, output, execution_result)
        structure_penalty = (1.0 - eval_result.structure_reward) * 2.0
        quality_penalty = (1.0 - eval_result.code_quality_reward) * 1.0
        
        total_error = total_error + structure_penalty + quality_penalty
        
        return torch.tensor(total_error / len(problem.train_pairs), device=device, requires_grad=True)
