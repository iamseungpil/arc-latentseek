"""
V14: Context-aware description optimization
- Maintains full context when optimizing description
- Uses proper forward pass through entire sequence
- Follows LatentSeek approach more closely
"""
...
class LatentOptimizerV14ContextAware:
    def __init__(
        self, 
        model, 
        tokenizer,
    ...
    def optimize(self, problem_id: str, initial_code: str, target_outputs: List[np.ndarray]) -> Dict[str, Any]:
        """Optimize using context-aware approach."""
        logger.info(f"Starting V14 context-aware optimization for problem {problem_id}")
        ...
        for step in range(self.num_steps):
            ...
            # Forward pass to get hidden states WITH FULL CONTEXT
            with torch.no_grad():
                outputs = self.model(
                    input_ids=input_ids,
                    output_hidden_states=True,
                    use_cache=False  # Don't use cache to ensure fresh computation
                )
                
            # Get hidden states for FULL description
            last_hidden = outputs.hidden_states[-1]
            
            # Clone the full sequence hidden states
            full_hidden = last_hidden.clone()
            
            # Make only description part trainable
            desc_hidden = full_hidden[:, desc_start:desc_end, :].detach().requires_grad_(True)
            
            # Replace in full hidden
            full_hidden_optimizable = full_hidden.detach()
            full_hidden_optimizable[:, desc_start:desc_end, :] = desc_hidden
            
            # Create optimizer for description hidden states
            ...
            for cand_idx in range(self.num_candidates):
                # Get logits WITH FULL CONTEXT
                # Pass the full hidden states through lm_head
                full_logits = self.model.lm_head(full_hidden_optimizable)
                
                # Extract logits for description tokens only
                desc_logits = full_logits[:, desc_start:desc_end, :]
                
                # Sample tokens for description
                ...
                # Option 1: Generate from description end (like LatentSeek)
                # This is more efficient and maintains better coherence
                with torch.no_grad():
                    # Start generation from the end of description
                    prompt_tokens = new_tokens[:, :desc_end]
