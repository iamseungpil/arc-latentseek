#!/usr/bin/env python3
"""
CompressARC-style Loss-based LatentSeek Experiment
Uses direct loss optimization instead of policy gradient
"""
...
from src.optimizers.latent_optimizer_compress import CompressARCStyleOptimizer

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def run_compress_loss_experiment(
    num_problems: int = 10,
    device: str = "cuda",
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    output_dir: str = "results/compress_loss"
):
    """Run CompressARC-style loss-based LatentSeek experiment"""
    ...
    evaluator = MultiTensorEvaluator()  # For consistent evaluation
    
    # Initialize CompressARC-style optimizer
    optimizer = CompressARCStyleOptimizer(
        barc_generator=generator,
        code_executor=executor,
        lr=0.01,  # CompressARC uses 0.01
        max_steps=50,  # More steps for loss-based optimization
        k=0.2,
        kl_weight=0.1,  # Similar to CompressARC's KL weight
    ...
    # Results tracking
    results = {
        "experiment": "compress_loss_latentseek",
        "model": model_name,
        ...
        "optimizer_config": {
            ...
            "k": optimizer.k,
            "kl_weight": optimizer.kl_weight,
            "structure_weight": optimizer.structure_weight,
            "accuracy_weight": optimizer.accuracy_weight
        },
        "problems": []
    }
    ...
    for i, problem in enumerate(problems):
        ...
        try:
            ...
            # Run CompressARC-style optimization
            logger.info("\nRunning CompressARC-style loss optimization...")
            ...
            # Extract final loss trajectory info
            if opt_result.loss_history:
                problem_result["initial_loss"] = opt_result.loss_history[0]
                problem_result["final_loss"] = opt_result.loss_history[-1]
                problem_result["loss_reduction"] = opt_result.loss_history[0] - opt_result.loss_history[-1]
            ...
            # Save both initial and final code
            ...
            final_code_path = output_path / f"{problem.uid}_final_code.py"
            with open(final_code_path, 'w') as f:
                f.write(final_output.code)
                
            problem_result["initial_code_path"] = str(initial_code_path)
            problem_result["final_code_path"] = str(final_code_path)
