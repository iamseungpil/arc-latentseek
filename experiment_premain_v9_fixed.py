#!/usr/bin/env python3
"""
Pre-Main V9 Fixed Experiment
Optimizes concepts and description sections
"""
...
from src.optimizers.latent_optimizer_premain_v9_fixed import PreMainLatentOptimizerV9Fixed

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def run_premain_v9_fixed_experiment(
    num_problems: int = 10,
    device: str = "cuda",
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    output_dir: str = "results/premain_v9_fixed"
):
    """Run Pre-Main V9 Fixed experiment"""
    ...
    # Initialize V9 Fixed optimizer
    optimizer = PreMainLatentOptimizerV9Fixed(
    ...
    for i, problem in enumerate(problems):
        ...
        try:
            ...
            # Run V9 Fixed optimization
            logger.info("\nRunning Pre-Main V9 Fixed optimization...")
            ...
            problem_result["premain_history"] = opt_result.premain_history
            ...
            # Save pre-main evolution
            evolution_path = output_path / f"{problem.uid}_premain_evolution.txt"
            with open(evolution_path, 'w') as f:
                for i, premain in enumerate(opt_result.premain_history):
                    f.write(f"=== Step {i} ===\n")
                    f.write(premain)
                    f.write("\n\n")
            problem_result["premain_evolution_path"] = str(evolution_path)
    ...
    logger.info(f"\n{'='*80}")
    logger.info("V9 FIXED EXPERIMENT COMPLETE")
...
if __name__ == "__main__":
    ...
    run_premain_v9_fixed_experiment(
