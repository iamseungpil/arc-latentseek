#!/usr/bin/env python3
"""
Experiment with V17 - Proper LatentSeek implementation
"""
...
from src.optimizers.latent_optimizer_v17_proper_latentseek import LatentOptimizerV17ProperLatentSeek
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('experiment_v17.log')
    ]
)
logger = logging.getLogger(__name__)

def run_v17_experiment(
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    num_problems: int = 5,
    device: str = "cuda:5"
):
    """Run V17 experiment with proper LatentSeek implementation."""
    ...
    # Initialize V17 optimizer with LatentSeek parameters
    optimizer = LatentOptimizerV17ProperLatentSeek(
        model=model,
        tokenizer=tokenizer,
        evaluator=evaluator,
        learning_rate=0.03,  # Same as original LatentSeek
        num_steps=10,        # Same as original LatentSeek
        temperature=1.0,
        max_new_tokens=1024,
        grad_clip=None
    )
    ...
    # Results storage
    results = {
        "experiment": "v17_proper_latentseek",
    ...
    for i, problem_id in enumerate(problem_ids):
        ...
        # Run V17 optimization
        logger.info("Running V17 optimization with proper LatentSeek approach...")
        ...
        if opt_result["success"]:
            ...
            if opt_result["final_code"]:
                code_path = Path(f"results/v17/{problem_id}_final.py")
        ...
        with open("results/v17/results.json", 'w') as f:
    ...
    if completed:
        ...
        logger.info(f"\n{'='*80}")
        logger.info("V17 EXPERIMENT COMPLETE")
        ...
        logger.info(f"Results saved to: results/v17")
        logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    run_v17_experiment(device="cuda:5")