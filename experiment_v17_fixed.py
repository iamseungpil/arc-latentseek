#!/usr/bin/env python3
"""
Experiment with V17 Fixed - Proper token generation from description end
"""
...
from src.optimizers.latent_optimizer_v17_fixed import LatentOptimizerV17Fixed
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('experiment_v17_fixed.log')
    ]
)
logger = logging.getLogger(__name__)

def run_v17_fixed_experiment(
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    num_problems: int = 5,
    device: str = "cuda:5"
):
    """Run V17 Fixed experiment."""
    ...
    # Initialize V17 Fixed optimizer
    optimizer = LatentOptimizerV17Fixed(
        model=model,
        tokenizer=tokenizer,
        evaluator=evaluator,
        learning_rate=0.03,
    ...
    # Results storage
    results = {
        "experiment": "v17_fixed_proper_generation",
    ...
    for i, problem_id in enumerate(problem_ids):
        ...
        # Run V17 Fixed optimization
        logger.info("Running V17 Fixed optimization...")
        ...
        if opt_result["success"]:
            ...
            if opt_result["final_code"]:
                code_path = Path(f"results/v17_fixed/{problem_id}_final.py")
        ...
        with open("results/v17_fixed/results.json", 'w') as f:
    ...
    if completed:
        ...
        logger.info(f"\n{'='*80}")
        logger.info("V17 FIXED EXPERIMENT COMPLETE")
        ...
        logger.info(f"Results saved to: results/v17_fixed")
        logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    run_v17_fixed_experiment(device="cuda:5")