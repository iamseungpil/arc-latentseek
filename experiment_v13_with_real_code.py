#!/usr/bin/env python3
"""
Experiment with V13 using previously generated real code
"""
...
from src.optimizers.latent_optimizer_v13_proper_desc import LatentOptimizerV13ProperDesc
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('experiment_v13_real.log')
    ]
)
...
def main(input_grid):
    ...
    "4e45f183": """# concepts:
# pattern recognition, grid transformation, size reduction

# description:
# The input grid contains a repeating pattern of colored pixels.
# To create the output, identify the smallest repeating unit (the pattern)
# and output just one instance of this pattern. The pattern repeats both
# horizontally and vertically to fill the input grid.

def main(input_grid):
    h, w = input_grid.shape
    
    # Find the pattern size by checking divisors
    pattern_h, pattern_w = h, w
    ...
    for ph in range(1, h + 1):
        if h % ph == 0:
            is_pattern = True
            for i in range(ph, h):
                if not np.array_equal(input_grid[i % ph], input_grid[i]):
                    is_pattern = False
                    break
            if is_pattern:
                pattern_h = ph
    ...
    for pw in range(1, w + 1):
        if w % pw == 0:
            is_pattern = True
            for j in range(pw, w):
                if not np.array_equal(input_grid[:, j % pw], input_grid[:, j]):
                    is_pattern = False
                    break
            if is_pattern:
                pattern_w = pw
    ...
    # Extract the pattern
    output_grid = input_grid[:pattern_h, :pattern_w]
    return output_grid"""
}

def run_v13_real_code_experiment(
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    num_problems: int = 2,  # Limited to available real code
    device: str = "cuda:0"
):
    """Run V13 experiment with real pre-generated code."""
    ...
    # Initialize V13 optimizer
    optimizer = LatentOptimizerV13ProperDesc(
        model=model,
        tokenizer=tokenizer,
        evaluator=evaluator,
        num_candidates=8,
    ...
    for i, problem_id in enumerate(problem_ids):
        ...
        # Run V13 optimization
        logger.info("\nRunning V13 optimization on real code...")
        ...
        if opt_result["success"]:
            ...
            # Save final code
            code_path = Path(f"results/v13_real/{problem_id}_final.py")
        ...
        with open("results/v13_real/results.json", 'w') as f:
    ...
    if completed:
        ...
        logger.info(f"\n{'='*80}")
        logger.info("V13 REAL CODE EXPERIMENT COMPLETE")
        ...
        logger.info(f"Results saved to: results/v13_real")
        logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    run_v13_real_code_experiment(device="cuda:0")