#!/usr/bin/env python3
"""
Description-Only V9 Experiment
Optimizes only description section using autoregressive generation
"""
...
from src.optimizers.latent_optimizer_description_only_v9 import DescriptionOnlyLatentOptimizerV9

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def run_description_only_v9_experiment(
    num_problems: int = 10,
    device: str = "cuda",
    model_name: str = "barc0/Llama-3.1-ARC-Potpourri-Induction-8B",
    output_dir: str = "results/description_only_v9"
):
    """Run Description-Only V9 experiment"""
    ...
    optimizer = DescriptionOnlyLatentOptimizerV9(
        barc_generator=generator,
        code_executor=executor,
        lr=0.03,  # Same as LatentSeek
        max_steps=10,  # Same as LatentSeek
        # 5D weights
        accuracy_weight=0.3,
        color_weight=0.2,
        spatial_weight=0.2,
    ...
    # Results tracking
    results = {
        "experiment": "description_only_v9",
        "model": model_name,
        "timestamp": datetime.now().isoformat(),
        "optimizer_config": {
            "lr": 0.03,
            "max_steps": 10,
            "target": "description_only",
            "weights": {
                "accuracy": 0.3,
                "color": 0.2,
                "spatial": 0.2,
    ...
    for i, problem in enumerate(problems):
        ...
        try:
            ...
            # Run Description-Only V9 optimization
            logger.info("\nRunning Description-Only V9 optimization...")
            ...
            # Save description evolution
            problem_result["description_history"] = opt_result.description_history
            ...
            # Save description evolution
            evolution_path = output_path / f"{problem.uid}_description_evolution.txt"
            with open(evolution_path, 'w') as f:
                for i, desc in enumerate(opt_result.description_history):
                    f.write(f"=== Step {i} ===\n")
                    f.write(desc)
                    f.write("\n\n")
            problem_result["description_evolution_path"] = str(evolution_path)
    ...
    logger.info(f"\n{'='*80}")
    logger.info("DESCRIPTION-ONLY V9 EXPERIMENT COMPLETE")
...
if __name__ == "__main__":
    ...
    run_description_only_v9_experiment(
