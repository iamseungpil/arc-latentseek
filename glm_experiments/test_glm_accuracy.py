#!/usr/bin/env python3
"""
GLM Accuracy Test Experiments
Test GLM's evaluation accuracy with different approaches
"""
...
class GLMExperimentRunner:
    def __init__(self):
        ...
        self.model = Glm4vForConditionalGeneration.from_pretrained(
            ...
            device_map={"": 0}  # Will use CUDA_VISIBLE_DEVICES=5
        )
        print("GLM model loaded")
        
    def get_failed_problems(self) -> List[Tuple[str, str, str]]:
        """Get problems with 0% accuracy from logs"""
        # Problems with 0% accuracy but successful execution (no ERROR)
        return [
            ("2072aba6", "results_early_stop/logs/2072aba6_initial_candidate_1.txt",
             "In the input, you will see a 3x3 grid with a specific pattern of gray pixels. To make the output, you should repeat the pattern to fill a 6x6 grid, and change the gray pixels to blue and red in a checkerboard pattern."),
            ("bb52a14b", "results_early_stop/logs/bb52a14b_initial_candidate_2.txt", 
             ""),  # Will extract from log file
            ("136b0064", "results_early_stop/logs/136b0064_initial_candidate_1.txt",
             "")   # Will extract from log file
        ]
    
    def extract_code_and_description(self, log_path: str) -> Tuple[str, str]:
        """Extract code and description from log file"""
        with open(log_path, 'r') as f:
            content = f.read()
            
        # Extract code
        code_start = content.find("Generated code:")
        code_end = content.find("Execution results:")
        if code_start != -1 and code_end != -1:
            code = content[code_start:code_end].replace("Generated code:", "").strip()
        else:
            code = ""
    ...
    def run_experiment1_image_understanding(self, problem_id: str, code: str, description: str):
        ...
        image_path = f"glm_experiments/exp1_image_understanding/{problem_id}_comparison.png"
...
Based on the pattern from input->expected, suggest a corrected description that would solve this problem."""
        ...
        # Save results
        output_path = f"glm_experiments/exp1_image_understanding/{problem_id}_results.json"
        with open(output_path, 'w') as f:
            json.dump({
                ...
                "execution_details": {
                    ...
                    "output_shapes": [str(g.shape) if g is not None and hasattr(g, 'shape') else "None" for g in execution_result.output_grids]
                },
                "code": code,
                "results": results
            }, f, indent=2)
    ...
    def run_experiment2_row_separation(self, problem_id: str, code: str, description: str):
        ...
        for i, (train_pair, output_grid) in enumerate(zip(problem.train_pairs, execution_result.output_grids)):
            image_path = f"glm_experiments/exp2_row_separation/{problem_id}_example_{i}.png"
            
            # Create single row image for this example
            # Use PIL directly to create a 3-column image
            cell_size = 30
            padding = 10
            gap = 20
            ...
            # Handle case where output_grid might be None or not a numpy array
            ...
            # Ensure output_grid is a valid numpy array
            if output_grid is not None and hasattr(output_grid, 'shape'):
                output_display = output_grid
            else:
                # Create error grid if output is invalid - use 255 as error indicator for uint8
                output_display = np.full(train_pair.y.shape, 255, dtype=np.uint8)
...
For each example, explain why the generated output differs from expected.""",
        ...
        # Save results
        output_path = f"glm_experiments/exp2_row_separation/{problem_id}_results.json"
        with open(output_path, 'w') as f:
            json.dump({
                "problem_id": problem_id,
                "original_description": description,
    ...
    def run_experiment3_multitensor(self, problem_id: str, code: str, description: str):
        ...
        image_path = f"glm_experiments/exp3_multitensor/{problem_id}_comparison.png"
        self.renderer.render_problem_with_output(problem, execution_result.output_grids, image_path)
...
Explanation: [brief reason]"""
        ...
        # Save results
        output_path = f"glm_experiments/exp3_multitensor/{problem_id}_results.json"
        with open(output_path, 'w') as f:
            json.dump({
                "problem_id": problem_id,
                "original_description": description,
    ...
    def run_all_experiments(self):
        """Run all experiments on selected problems"""
        problems = self.get_failed_problems()
        
        for problem_id, log_path, description in problems:
            print(f"\n{'='*60}")
            ...
            if not description:
                code, description = self.extract_code_and_description(log_path)
            else:
                with open(log_path, 'r') as f:
                    content = f.read()
                    code_start = content.find("from common import")
                    code_end = content.find("Execution results:")
                    code = content[code_start:code_end].strip() if code_start != -1 else ""
        ...
        print("Results saved in glm_experiments/ subdirectories")


if __name__ == "__main__":
    runner = GLMExperimentRunner()
